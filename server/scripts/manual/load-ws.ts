...
async function runHttpBenchmark(config: BenchConfig): Promise<void> {
  ...
  const started = performance.now();
  const workers = Array.from({ length: config.httpWorkers }, () =>
...
async function runWsBenchmark(config: BenchConfig): Promise<void> {
  ...
  const workers = Array.from({ length: config.wsClients }, (_, i) => runWsWorker(i + 1, config));
...
async function main(): Promise<void> {
  ...
  const config: BenchConfig = {
    ...
    wsConnectionsPerClient: parseNumber(
      "ws-connections-per-client",
      "LOAD_WS_CONNECTIONS_PER_CLIENT",
      8,
    ),
    wsRequestsPerConnection: parseNumber(
      "ws-requests-per-connection",
      "LOAD_WS_REQUESTS_PER_CONNECTION",
      20,
    ),
    wsDropRate: parseNumber("ws-drop-rate", "LOAD_WS_DROP_RATE", 0.1),
