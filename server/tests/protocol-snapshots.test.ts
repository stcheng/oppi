 ...
 * compatibility. If this test fails, the protocol contract has changed.
 *
...
import type { ServerMessage, Session, SessionChangeStats } from "../src/types.js";
  ...
  return {
    ...
    permission_request: {
      ...
      resolutionOptions: {
        allowSession: true,
        allowAlways: true,
        alwaysDescription: "Always allow rm commands",
        denyAlways: true,
      },
    },
    permission_expired: {
...
describe("protocol snapshots", () => {
  ...
  it("covers every ServerMessage type discriminator", () => {
    ...
    const expectedTypes = [
      "connected", "stream_connected", "state", "session_ended",
