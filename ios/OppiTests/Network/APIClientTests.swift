...
struct APIClientTests {
    ...
    @Test func getAppletDecodesLatestVersionPayload() async throws {
        let client = makeClient()
        defer { cleanup() }

        MockURLProtocol.handler = { request in
            #expect(request.url?.path == "/workspaces/w1/applets/a1")
            return self.mockResponse(json: """
            {
              "applet": {
                "id": "a1",
        ...
        let (applet, latestVersion) = try await client.getApplet(workspaceId: "w1", appletId: "a1")
        #expect(applet.id == "a1")
        #expect(applet.currentVersion == 3)
        #expect(latestVersion?.version.version == 3)
        #expect(latestVersion?.html == "<html>hi</html>")
    }

    @Test func createAppletEditSessionUsesWorkspaceScopedEndpoint() async throws {
        let client = makeClient()
        defer { cleanup() }

        MockURLProtocol.handler = { request in
            #expect(request.httpMethod == "POST")
            #expect(request.url?.path == "/workspaces/w1/applets/a1/edit-session")
            return self.mockResponse(json: """
            {
              "session": {
                "id": "edit-1",
              ...
              "provenance": {
                ...
                "sourceSessionName": "Build inspector"
              }
            }
            """)
        }

        let session = try await client.createAppletEditSession(workspaceId: "w1", appletId: "a1")
        #expect(session.id == "edit-1")
        #expect(session.workspaceId == "w1")
        #expect(session.name == "Edit: Inspector")
    }
