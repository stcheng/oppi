...
//   bun scripts/testflight.ts submit-external [build] [group] # submit an already-uploaded build
//
// Prerequisites:
...
async function cmdSubmitExternal(buildNumber?: string, groupName?: string): Promise<void> {
  ...
  console.log(`Adding build ${build.attributes.version} to "${group}"...`);
  await addBuildToGroup(groupId, build.id);

  console.log(`Submitting build ${build.attributes.version} for beta review...`);
  await submitForBetaReview(build.id);

  console.log(`\nBuild ${build.attributes.version} submitted to "${group}" for external beta review.`);
}
...
async function cmdBuild(opts: {
  ...
  const { bump, buildOnly, submitExternal, externalGroup, explicitBuild } = opts;
...
<dict>
  ...
  if (submitExternal && !buildOnly) {
    ...
    console.log(`Build ${newBuild} submitted to "${externalGroup}" for external beta review.`);
  }
...
function printHelp(): void {
  console.log(`testflight.ts — Archive, upload to App Store Connect, optionally submit to external beta.

Build commands:
  bun testflight.ts --bump                            Bump build number, archive, upload
  bun testflight.ts --bump --submit-external          + submit to "${DEFAULT_GROUP}"
  bun testflight.ts --bump --submit-external "Group"  + submit to named group
...
Standalone commands:
  ...
  bun testflight.ts submit-external [build] [group]   Submit an already-uploaded build
...
async function main(): Promise<void> {
  ...
  if (args[0] === "submit-external") {
    await cmdSubmitExternal(args[1], args[2]);
    return;
  }
  ...
  await cmdBuild({ bump, buildOnly, submitExternal, externalGroup, explicitBuild });
}
